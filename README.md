...
3. 결제가 되면 배달 시작한다
4. 고객이 주문을 취소할 수 있다
5. 주문이 취소되면 결제, 배송, 평가가 취소된다.
6. 고객이 주문상태를 중간중간 조회한다
7. 배송이 완료되면 쿠폰이 지급되고 평가가 진행된다.

비기능적 요구사항
1. 트랜잭션
    1. 주문이 완료되어야 결제가 가능하다.  Sync 호출 
    2. 배달이 완료되어야 평가가 가능하다.  Sync 호출 
2. 장애격리
    ...
    3. 평가취소기능이 수행되지 않더라도 배달은 365일 24시간 받을 수 있어야 한다  Async (event-driven), Eventual Consistency
    4. 배달시스템이 과중되면 평가는 잠시동안 받지 않고 평가를 잠시후에 하도록 유도한다  Circuit breaker, fallback
3. 성능
    1. 고객이 평가에 대한 상태를 시스템에서 확인할 수 있다 CQRS
...
- 동기식 호출에서는 호출 시간에 따른 타임 커플링이 발생하며, 평가시스템이 장애가 나면 배달도 못받는다는 것을 확인:
